...
export function UploadDialog({ open, onClose, periods, onSynced }: UploadDialogProps) {
  ...
  function handleFiles(files: FileList | null) {
    ...
    if (!f.name.match(/\.(xlsx|xls|xlsm|pdf)$/i)) {
      setParseError("Please upload a Crystal Report PDF (.pdf) or an Excel file (.xlsx, .xls, .xlsm).")
      return
    }
  ...
  async function handleParse() {
    ...
    if (!file) { setParseError("Please upload a file."); return }
    setParsing(true); setParseError(null)
    try {
      let result: ParseResult

      if (file.name.toLowerCase().endsWith(".pdf")) {
        // ── PDF path: server-side parser (pdfjs-dist) ──────────────────────
        ...
        // ── Excel path: existing client-side SheetJS parser ────────────────
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border shrink-0">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground font-medium">Crystal Report PDF or Excel Import</p>
            </div>
          </div>
          <button
            onClick={handleClose}
        ...
        <div className="flex-1 overflow-y-auto">
          {step === 1 && (
            <div className="p-6 space-y-5">
              ...
              <div>
                <label className=”block text-xs font-bold uppercase tracking-wider text-[#49426E] mb-1.5”>
                  Attendance File <span className=”text-red-400”>*</span>
                </label>
                <div
                ...
                >
                  ...
                  {file ? (
                    ...
                    <>
                      ...
                      <p className=”font-semibold text-[#322E53] text-sm”>Drop your Crystal Report PDF here</p>
                      <p className=”text-xs text-muted-foreground font-medium mt-1”>
                        or click to browse — .pdf, .xlsx, .xls, .xlsm supported
                      </p>
                    </>
                  )}
                </div>
              ...
              <div className=”p-3 rounded-xl bg-[#F5F4F8] border border-border text-xs text-muted-foreground font-medium leading-relaxed”>
                ...
                <p>
                  <b>Crystal Report PDF (recommended):</b> Upload the PDF directly from Union Developers
                  HR. All departments across all pages are parsed automatically. Only the HCM ID is
                  used to match employees — name and designation are ignored.
                </p>
                <p className=”mt-0.5”>
                  <b>Crystal Report Excel:</b> Same report exported as .xlsx — auto-detected.
                </p>
                <p className=”mt-0.5”>
